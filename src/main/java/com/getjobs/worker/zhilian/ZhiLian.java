...
public class ZhiLian {
    ...
    public void setCompanyNameFilter(String companyNameFilter) {
        this.companyNameFilter = normalize(companyNameFilter);
    }
    ...
    public int execute() {
        ...
        try {
            ...
            if (loginRequired) {
                String msg = "智联招聘需要先登录，请在打开的浏览器中完成登录后再继续投递";
                log.warn(msg);
                sendProgress(msg, null, null);
            } else {
        ...
        try {
            ...
            while (pageNum <= 50) {
                ...
                if (isLoginPopupVisible()) {
                    log.warn("login popup detected, stopping");
                    loginRequired = true;
                    isLimit = true;
                    return;
                }
        ...
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (companyName == null || companyName.isBlank()) {
                    companyName = safeGetText(card, "a.companyinfo__name");
                }
                if (companyName == null || companyName.isBlank()) {
                    companyName = safeGetText(card, "[class*='company'] a");
                }
                if (companyName == null || companyName.isBlank()) {
                    companyName = safeGetText(card, "[class*='companyname']");
                }
                ...
                jobs.add(new PageJob(i, jobId, jobTitle, normalize(companyName)));
            }
            ...
            for (PageJob pj : jobs) {
                ...
                // 如果设置了公司名过滤，检查公司名是否匹配
                if (companyNameFilter != null && !companyNameFilter.isBlank()) {
                    if (pj.companyName == null || !pj.companyName.contains(companyNameFilter)) {
                        log.info("岗位【{}】公司【{}】不匹配目标公司【{}】，跳过",
                            pj.jobTitle, pj.companyName, companyNameFilter);
                        continue;
                    }
                    log.info("岗位【{}】公司【{}】匹配目标公司，进入详情页投递",
                        pj.jobTitle, pj.companyName);
                }

                // 点击岗位标题进入详情页
                Locator card = page.locator("div.joblist-box__item").nth(pj.index);
                Locator titleLink = card.locator("a.jobinfo__name");
                if (titleLink.count() == 0) {
                    log.info("岗位【{}】未找到标题链接，跳过", pj.jobTitle);
                    continue;
                }

                try {
                    sendProgress("正在查看岗位: " + pj.jobTitle, null, null);
                    titleLink.first().click();
                    PlaywrightUtil.sleep(2);

                    // 在详情页查找投递按钮
                    boolean applied = applyFromDetailPage(pj);
                    ...
                    // 如果不自动提交，只处理一个岗位就停止
                    if (!autoSubmit) {
                        sendProgress("非自动提交模式，已填写申请并暂停，等待手动确认", null, null);
                        return false;
                    }
                } catch (Exception clickEx) {
                    log.warn("投递岗位【{}】失败，继续下一个: {}", pj.jobTitle, clickEx.getMessage());
                    // 确保回到搜索结果页
                    try { page.goBack(); PlaywrightUtil.sleep(1); } catch (Exception ignored) {}
                }
     ...
     * @return 是否成功点击投递
     */
    private boolean applyFromDetailPage(PageJob pj) {
        // 多候选选择器：详情页的投递按钮
        String[] applySelectors = new String[] {
            "button.btn-apply",
        ...
        if (applyBtn == null) {
            log.info("岗位【{}】详情页未找到投递按钮", pj.jobTitle);
            return false;
        }
        ...
        try {
            ...
            // 检查是否出现登录弹窗（未登录状态）—— 立即停止
            if (isLoginPage() || isLoginPopupVisible()) {
                log.warn("login required on detail page");
                loginRequired = true;
                isLimit = true;
                return false;
            }
            ...
            if (isApplicationForm()) {
                log.info("岗位【{}】出现申请表单", pj.jobTitle);
                if (!autoSubmit) {
                    // 非自动提交模式：填写表单后暂停
                    sendProgress("已打开申请表单，请手动确认并提交", null, null);
                    return true;
                }
                // 自动提交模式：尝试提交
            ...
            log.info("岗位【{}】投递操作完成", pj.jobTitle);
            return true;
        } catch (Exception e) {
            log.warn("详情页投递操作失败: {}", e.getMessage());
            return false;
        } finally {
    ...
    private boolean isLoginPage() {
        try {
            String url = page.url();
            if (url.contains("passport") || url.contains("login")) return true;
            Locator loginDialog = page.locator("div.login-dialog, div[class*='login-modal'], div[class*='qrcode-login']");
            return loginDialog.count() > 0;
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private boolean isApplicationForm() {
        try {
            Locator form = page.locator("div.apply-form, div[class*='delivery-dialog'], div[class*='apply-workflow']");
            return form.count() > 0;
        } catch (Exception e) {
            return false;
        }
    ...
    private void submitApplicationForm() {
        try {
            ...
            log.warn("提交申请表单失败: {}", e.getMessage());
        }
    }
     ...
     */
    /**
     * Normalize whitespace: trim, collapse newlines/tabs/multiple spaces into single space.
     */
    ...
    /**
     * Detect login popup without trying to dismiss it.
     */
    private boolean isLoginPopupVisible() {
        try {
            Locator popup = page.locator("div.a-modal.login-popups");
            return popup.count() > 0 && popup.first().isVisible();
        } catch (Exception e) {
            return false;
        }
    }
