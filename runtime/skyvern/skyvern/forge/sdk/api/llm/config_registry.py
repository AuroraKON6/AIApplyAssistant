...
if settings.ENABLE_ANTHROPIC:
    ...
    # Xiaomi MiMo model via local HTTP proxy (bypasses Python TLS issues)
    LLMConfigRegistry.register_config(
        "MIMO_V2_5_PRO",
        LLMConfig(
            "openai/mimo-v2.5-pro",
            [],  # Optional local proxy config; user supplies the key in runtime .env
            supports_vision=False,  # MiMo API does not support image input
            add_assistant_prefix=False,
