...
public class ZhilianController {
     ...
     */
    @PostMapping("/start")
    public ResponseEntity<Map<String, Object>> startZhilianJob(@RequestBody(required = false) Map<String, Object> body) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            if (body != null) {
                keyword = body.get("keyword") != null ? body.get("keyword").toString() : null;
                companyName = body.get("companyName") != null ? body.get("companyName").toString() : null;
                Object autoSubmitObj = body.get("autoSubmit");
                if (autoSubmitObj instanceof Boolean) {
                    autoSubmit = (Boolean) autoSubmitObj;
                }
            }

            final String kw = keyword;
            final String cn = companyName;
            final boolean as = autoSubmit;

            // 异步启动新任务
            CompletableFuture.runAsync(() -> {
                zhilianJobService.executeDelivery(kw, cn, as, progressMessage -> {
